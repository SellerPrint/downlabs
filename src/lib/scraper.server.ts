...
async function harvestSeed(seedUrl: string) {
  ...
  for (const appUrl of unique) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.from("apps").upsert(row, { onConflict: "uptodown_url" });
      if (!error) inserted += 1;
    } catch (e) {
...
export async function runScrape(seeds: string[] = DEFAULT_SEEDS) {
  ...
  return { ok: !errorMsg, found: totalFound, inserted: totalInserted, error: errorMsg };
}
